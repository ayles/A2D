...
public:
    ...
    template<class T>
    typename std::enable_if<std::is_base_of<a2d::Component, T>::value, SMART_TYPE(T)>::type
    AddComponent() {
        std::type_index t_index = typeid(T);
        SMART_TYPE(T) component = new T;
    ...
    template<class T>
    typename std::enable_if<std::is_base_of<Component, T>::value, SMART_TYPE(T)>::type
    GetComponent() const {
