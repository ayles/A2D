...
void TextureBuffer::SetRect(int x, int y, const TextureBuffer &rect) {
    if (x < 0 || x + rect.width > width || y < 0 || y + rect.height > height) {
        LOG_TRACE("Can't fit rect");
        return;
    }
    if (!rect.IsAllocated()) {
        LOG_TRACE("Rect is empty");
        return;
    }
    Allocate();
    for (int i = 0; i < rect.width; ++i) {
        for (int j = 0; j < rect.height; ++j) {
            int x_ = x + i;
            int y_ = y + j;
            int offset = (j * rect.width + i) * 4;
            int offset_ = (y_ * width + x_) * 4;
            data[offset_] = rect.data[offset];
            data[offset_ + 1] = rect.data[offset + 1];
            data[offset_ + 2] = rect.data[offset + 2];
            data[offset_ + 3] = rect.data[offset + 3];
        }
    }
}
